...
import type { TournamentSnapshot, Group, Match, MatchPrediction, BracketMatch } from '../../lib/types'
...
describe('Frontend Integration: Predictions Flow', () => {
  ...
  const mockTournamentSnapshot: TournamentSnapshot = {
    ...
    bracket: [
      {
        ...
        prediction: {
          ...
          reasoning: 'USA shows strong form with higher xG average and home advantage.',
          confidence: 'high'
        }
      },
      {
    ...
    aiSummary: 'USA and Brazil emerge as tournament favorites with strong group stage performances.',
  ...
  describe('fetchLatestPredictions() integration', () => {
    it('should return correct TournamentSnapshot structure', async () => {
      ...
      vi.mocked(fetchLatestPredictions).mockResolvedValueOnce(mockTournamentSnapshot)
    ...
    it('should detect stale data (>2 hours old)', async () => {
      const staleTimestamp = new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString() // 3 hours ago
  ...
  describe('MatchCard component with predictions', () => {
    it('should display match details and prediction', () => {
      ...
      const { container } = renderWithIntl(<MatchCard match={match} prediction={prediction} />)
      ...
      expect(container.textContent?.includes('68') || container.textContent?.includes('0.68')).toBeTruthy()
    })
  ...
  describe('Error handling', () => {
    it('should handle Firestore fetch errors', async () => {
      // Mock fetch error
      vi.mocked(fetchLatestPredictions).mockRejectedValueOnce(new Error('Firestore connection failed'))

      await expect(fetchLatestPredictions()).rejects.toThrow('Firestore connection failed')
    })
    ...
    it('should handle malformed snapshot data', async () => {
      ...
      vi.mocked(fetchLatestPredictions).mockResolvedValueOnce(invalidSnapshot as any)
  ...
  describe('Full integration: Firestore → Components', () => {
    it('should render complete UI from fetched data', async () => {
      ...
      const { container: groupContainer } = renderWithIntl(<GroupCard group={data!.groups.A} />)
      expect(groupContainer.textContent).toContain('USA')
      expect(groupContainer.textContent).toContain('Mexico')

      const { container: bracketContainer } = renderWithIntl(<BracketView bracket={data!.bracket} />)
