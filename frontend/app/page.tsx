		...
		return allMatches
			.filter((match) => {
				...
				return favoriteTeams.some(
					(fav) =>
						homeTeam.includes(fav) ||
						awayTeam.includes(fav) ||
						fav.includes(homeTeam) ||
						fav.includes(awayTeam),
				);
			})
			.sort((a, b) => {
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50">
			...
			<div className="fixed inset-0 opacity-5 pointer-events-none">
				...
				<div
					className="absolute bottom-0 right-1/4 w-96 h-96 bg-yellow-300 rounded-full blur-3xl animate-pulse"
			...
			<header className="sticky top-0 z-50 backdrop-blur-xl bg-white/95 border-b border-emerald-200 shadow-lg">
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					...
					<div className="flex flex-col items-center py-4 sm:py-6 gap-3 sm:gap-4">
						...
						<nav
							className="w-full sm:w-auto flex bg-slate-100 p-1 rounded-2xl border border-slate-200 shadow-md overflow-x-auto"
							role="tablist"
							aria-label="Turneringsvisninger"
						>
							{(["favorites", "groups", "matches", "bracket"] as Tab[]).map(
								(tab, index) => (
									<button
										key={tab}
			...
			<main className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12">
				...
				{loading && <LoadingSpinner message={t("loadingPredictions")} />}
				...
				{error && !loading && (
					<div className="bg-red-50 border-2 border-red-300 rounded-2xl p-6 sm:p-8 text-center shadow-xl">
						...
						<div className="text-red-800 font-bold text-lg sm:text-xl mb-2">
							{t("errorLoading")}
						</div>
									...
									<div className="max-w-2xl mx-auto">
										<div className="p-12 bg-gradient-to-br from-slate-50 to-slate-100 rounded-3xl border-2 border-slate-200 text-center">
											...
											<p className="text-slate-600 mb-6 max-w-md mx-auto">
												Klikk på stjerneikonet i gruppetabellene for å følge
												dine favorittlag og se alle deres kamper her
											</p>
											<button
												...
												<div className="flex items-center gap-3">
													...
													<div>
														...
														<p className="text-sm text-amber-700">
															...
															{favoriteMatches.length === 1 ? "kamp" : "kamper"}
														</p>
													</div>
												</div>
											</div>
											...
											<div className="flex flex-wrap gap-3">
												{favoriteTeams.map((team) => {
													const teamMatches = favoriteMatches.filter(
														(m) =>
															m.homeTeamName?.includes(team) ||
															m.awayTeamName?.includes(team),
													);
													return (
														<div
															key={team}
														...
														>
															...
															<div className="text-xs text-amber-700">
																{teamMatches.length}{" "}
																{teamMatches.length === 1 ? "kamp" : "kamper"}
															</div>
														</div>
													);
												})}
							...
							>
								...
								<div className="relative overflow-hidden bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-8 border-2 border-emerald-200 shadow-xl">
									...
									<div className="relative z-10">
										...
										<p className="text-slate-700 leading-relaxed text-sm sm:text-base mb-6 sm:mb-8">
											{snapshot.aiSummary}
										</p>
											...
											<div className="bg-gradient-to-br from-emerald-50 to-emerald-100 p-4 sm:p-6 rounded-xl border-2 border-emerald-300">
												...
												<ul className="space-y-2">
													{snapshot.favorites.map((team) => (
														<li
															key={team}
															className="text-slate-800 font-semibold text-sm sm:text-base flex items-center gap-2"
														>
