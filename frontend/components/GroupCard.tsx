				...
				<table className="w-full text-sm">
					<thead>
						<tr className="bg-emerald-50 border-b border-emerald-200">
							<th
								className="px-4 py-3 text-left text-xs font-bold text-emerald-800 uppercase tracking-wider"
								title={t("rank")}
							>
								#
							</th>
							...
							<th
								className="px-2 py-3 text-center text-xs font-bold text-emerald-800 uppercase tracking-wider"
								title={t("playedFull")}
							>
								{t("played")}
							</th>
							...
							<th
								className="hidden sm:table-cell px-2 py-3 text-center text-xs font-bold text-emerald-800 uppercase tracking-wider"
								title={t("goalsForFull")}
							>
								{t("goalsFor")}
							</th>
							<th
								className="hidden sm:table-cell px-2 py-3 text-center text-xs font-bold text-emerald-800 uppercase tracking-wider"
								title={t("goalsAgainstFull")}
							>
								{t("goalsAgainst")}
							</th>
							<th
								className="px-2 py-3 text-center text-xs font-bold text-emerald-800 uppercase tracking-wider"
								title={t("goalDifferenceFull")}
							>
								{t("goalDifference")}
							</th>
							<th
					...
					<tbody className="divide-y divide-gray-100">
						{sortedTeams.map((team, index) => {
							...
							return (
								<tr
									key={`${group.letter}-${team.name}-${index}`}
									className={rowClass}
								>
									{/* Rank */}
										...
										<div className="flex items-center justify-between gap-2">
													...
													{team.predictedRank && (
														<div className="flex items-center gap-1 mt-0.5">
															...
															<span
																...
																title={t("aiPredictedRank", {
																	rank: team.predictedRank,
																})}
															>
																{team.predictedRank}.
															</span>
											...
											>
												{favorites.includes(team.name) ? (
													...
													<svg
														className="w-5 h-5 text-slate-400 hover:text-yellow-500"
														fill="none"
														viewBox="0 0 20 20"
														stroke="currentColor"
													>
