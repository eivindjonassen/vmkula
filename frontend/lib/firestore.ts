...
export async function fetchLatestPredictions(opts?: {
	...
	if (!forceRefresh && cache) {
		...
		console.log(
			`⚠️ Cache STALE (age: ${Math.round(cacheAge / 1000)}s, TTL: ${CACHE_TTL_MS / 1000}s)`,
		);
	} else {
...
async function fetchFromFirestore(): Promise<TournamentSnapshot | null> {
	try {
		...
		console.log(
			"📊 Matches in raw data:",
			rawData.matches ? rawData.matches.length : "NO MATCHES FIELD",
		);
		console.log(
			"📊 Bracket in raw data:",
			rawData.bracket ? rawData.bracket.length : "NO BRACKET FIELD",
		);
		console.log(
			"📊 Predictions in raw data:",
			rawData.predictions ? rawData.predictions.length : "NO PREDICTIONS FIELD",
		);
		...
		if (data.updatedAt) {
			...
			if (ageMs > STALE_THRESHOLD_MS) {
				console.warn(
					`⚠️ Data is stale (age: ${Math.round(ageMs / 1000 / 60)}min, threshold: ${STALE_THRESHOLD_MS / 1000 / 60}min)`,
				);
